...
import sys, os, multiprocessing, importlib, pickle, time
from multiprocessing.pool import Pool  
from tqdm.auto import tqdm
from torchvision.transforms import Normalize

from util import *   # STA: local trimmed copy (keras imports removed)


import warnings
warnings.filterwarnings("ignore")

# STA: torch_disca_run.py's original code inserted the channel axis at
# position -1 (np.expand_dims(v, -1)), giving shape (n, D, H, W, 1) instead of
# proper NCDHW (n, 1, D, H, W). Since Conv3d always reads dim 1 as the channel
# count, this made the D axis (size = box, e.g. 32) get treated as 32 input
# channels, and the true channel axis (size 1) got treated as a spatial
# dimension of extent 1 -- silently corrupting the 3D convolution, and only
# "working" at box=32 because YOPOFeatureModel.m1 happens to hardcode
# in_channels=32. Gated behind this env var (default off) so every existing
# box=32 baseline result stays byte-for-byte reproducible; set
# DISCA_FIX_CHANNELS=1 to use the corrected NCDHW shape at any box size.
FIX_CHANNELS = os.environ.get('DISCA_FIX_CHANNELS', '0') == '1'

# configuration
class Config:
    data_dir = ''
    image_size = 32  ### subtomogram size ###
    # STA: candidate cluster counts driven by env DISCA_K (e.g. "2" or "2,3,4")
    candidateKs = [int(_) for _ in os.environ.get('DISCA_K', '2').split(',')]

    batch_size = int(os.environ.get('DISCA_BATCH_SIZE', '64'))
    ...
    # STA: per-run output dir/tag from env DISCA_TAG (default reflects K list)
    _tag = os.environ.get('DISCA_TAG', 'k' + '_'.join(map(str, candidateKs)))
    _outdir = os.environ.get('DISCA_OUTDIR', './model')
    model_path = os.path.join(_outdir, f'model_torch_{_tag}.pth')   ### saved torch model ###
    label_path = os.path.join(_outdir, f'labels_{_tag}.pickle')     ### saved cluster labels ###

    # STA: single RTX 5080 -> cuda:0 (stock script hard-coded cuda:2)
...
class YOPOFeatureModel(nn.Module):
    ...
    def __init__(self):
        ...
        m1_in = 1 if FIX_CHANNELS else 32
        self.m1 = self.get_block(m1_in, 64)
...
if __name__ == '__main__':  
    ...
    # STA: input pickle path from env DISCA_INPUT
    x_train = os.environ.get('DISCA_INPUT', '')
    os.makedirs(Config._outdir, exist_ok=True)
    print(f'DISCA_INPUT={x_train}  candidateKs={Config.candidateKs}  device={Config.device}')
    ...
    x_train = [np.expand_dims(data_sv['vs'][_]['v'], 0 if FIX_CHANNELS else -1) for _ in x_keys]
    data_array_normalized = np.array(x_train)
    if os.environ.get('DISCA_PERPARTICLE_NORM') == '1':
        # image_normalization() above (per-particle Normalize(mean,std)) is defined
        # but never called anywhere in this pipeline -- dead code, no per-particle
        # contrast normalization actually happens. Apply it here instead.
        n = data_array_normalized.shape[0]
        flat = data_array_normalized.reshape(n, -1)
        mean = flat.mean(axis=1).reshape([n] + [1] * (data_array_normalized.ndim - 1))
        std = flat.std(axis=1).reshape([n] + [1] * (data_array_normalized.ndim - 1))
        data_array_normalized = (data_array_normalized - mean) / (std + 1e-8)
